...
<x-modal :name="__('asientoModal')" :size="__('xl')" :show="false" :title="__('Nuevo asiento contable')"
  ...
  <button type="button" class="btn btn-secondary mb-3"
    id="addCuentaBtn">Agregar Cuenta</button>
      ...
      $(`.account-select-${index}`).select2({
        ...
        ajax: {
          ...
          data: function(params) {
            return {
              ...
              page: params.page || 1,
              template_id: "{{ $exercise->template_id }}" // Pasar el template_id del ejercicio actual
            };
          },
        ...
        templateResult: function(data) {
          ...
          switch (data.signo) {
            case 'P':
              icon =
                '<i class="fas fa-solid fa-circle-plus text-success"></i>';
              break;
            case 'D':
              icon =
                '<i class="fas fa-solid fa-circle-minus text-danger"></i>';
              break;
            case 'N':
              icon =
                '<i class="fas fa-solid fa-circle-dot text-warning"></i>';
              break;
            default:
              icon = '';
              break;
