                        ...
                        <form id="settings-form" class="settings-form">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="cuenta" name="cuenta"
                                    placeholder="Escriba..." value="{{ old('cuenta') }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="setting-input-2" class="form-label">Descripcion</label>
                                <input type="text" class="form-control" id="description" name="description"
                                    placeholder="Escriba..." value="{{ old('description') }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="signo" class="form-label">Signo</label>
                                <select class="form-control" id="signo" name="signo" required>
                                    <option value="P" @if (old('signo') == 'P') selected @endif>
                                        Positivo</option>
                                    <option value="D" @if (old('signo') == 'D') selected @endif>Doble
                                    </option>
                                    <option value="N" @if (old('signo') == 'N') selected @endif>Negativo
                                    </option>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="tipocuenta" name="tipocuenta" required>
                                    <option value="T" @if (old('tipocuenta') == 'T') selected @endif>
                                        Total</option>
                                    <option value="D" @if (old('tipocuenta') == 'D') selected @endif>Detalle
                                    </option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="tipoestado" class="form-label">Tipo de Estado</label>
                                <select class="form-control" id="tipoestado" name="tipoestado" required>
                                    <option value="1" @if (old('tipoestado') == '1') selected @endif>
                                        ESTADO DE SITUACIÓN FINANCIERA</option>
                                    <option value="2" @if (old('tipoestado') == '2') selected @endif>ESTADO DE
                                        RESULTADO INTEGRAL
                                    </option>
                                    <option value="3" @if (old('tipoestado') == '3') selected @endif>ESTADO DE
                                        FLUJOS DE EFECTIVO
                                    </option>
                                    <option value="5" @if (old('tipoestado') == '5') selected @endif>ESTADO DE
